defmodule TunezWeb.Artists.ShowLiveTest do
  ...
  describe "album_details/1" do
    ...
    @tag skip: "Can be enabled during chapter 8. Also need to change `_conn` to `conn` below"
  ...
  describe "events" do
    test "can delete artists", %{conn: conn} do
      ...
      assert {:error, _error} = Music.read_artist(artist.id)
    end
    ...
    test "can delete albums", %{conn: conn} do
      ...
      assert {:error, _error} = Music.read_album(album.id)
    end
  end
end
