defmodule TunezWeb.Graphql.ArtistTest do
  ...
  describe "queries" do
    test "getArtistById" do
      ...
      assert resp.data["artist"]["name"] == "Test Name"
      assert resp.data["artist"]["albums"] == [%{"name" => album.name}]
    end
    ...
    test "searchArtists" do
      ...
      assert {:ok, resp} =
               """
               query artists($query: String, $sort: [ArtistSortInput]) {
                 artists(query: $query, sort: $sort) {
                   results {
                   name}
                 }
               }
      ...
      names = Enum.map(resp.data["artists"]["results"], & &1["name"])
