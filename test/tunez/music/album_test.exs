defmodule Tunez.Music.AlbumTest do
  ...
  describe "Tunez.Music.create_album/1-2" do
    test "stores the actor that created the record" do
      ...
      album =
        Music.create_album!(
          %{name: "New Album", artist_id: artist.id, year: 2024},
          actor: actor
        )
  ...
  describe "validations" do
    test "year must be between 1950 and now" do
      admin = generate(user(role: :admin))
      artist = generate(artist())

      assert Music.create_album!(
               %{artist_id: artist.id, name: "test 2024", year: 2024},
               actor: admin
             )
      ...
      %{artist_id: artist.id, name: "test 1950", year: 1950}
      |> Music.create_album(actor: admin)
      |> assert_has_error(Ash.Error.Invalid, fn
        %{message: message} ->
          message == "must be greater than %{greater_than} and must be less than or equal to %{less_than_or_equal_to}"
    ...
    test "image_url must be either a remote URL or a local URL from /images" do
      admin = generate(user(role: :admin))
      artist = generate(artist())
      ...
      assert_raise Ash.Error.Invalid, ~r/must be valid url/, fn ->
      ...
      |> assert_has_error(fn error ->
        error.field == :image_url && error.message == "must be valid url"
      end)
    end
  end
end
