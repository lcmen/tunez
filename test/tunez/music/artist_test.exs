defmodule Tunez.Music.ArtistTest do
  ...
  describe "Tunez.Music.search_artists/1-2" do
    ...
    test "can sort by creation time" do
      generate(artist(seed?: true, name: "first", created_at: ago(30, :second)))
      generate(artist(seed?: true, name: "third", created_at: ago(10, :second)))
      generate(artist(seed?: true, name: "second", created_at: ago(20, :second)))

      actual = names(Music.search_artists!("", query: [sort_input: "-created_at"]))
    ...
    test "can sort by latest album release" do
      ...
      generate(album(year: 2023, artist_id: first.id))

      third = generate(artist(name: "third"))
      generate(album(year: 2008, artist_id: third.id))

      second = generate(artist(name: "second"))
      generate(album(year: 2012, artist_id: second.id))

      actual =
        names(Music.search_artists!("", query: [sort_input: "--latest_album_year"]))
    ...
    test "can sort by number of album releases" do
      ...
      actual =
        names(Music.search_artists!("", query: [sort_input: "-albums_count"]))
  ...
  describe "Tunez.Music.unfollow_artist/2" do
    test "deletes a specific ArtistFollower record" do
      ...
      artist_one |> Music.follow_artist!(actor: user_one)
      artist_one |> Music.follow_artist!(actor: user_two)

      artist_two |> Music.follow_artist!(actor: user_one)
      artist_two |> Music.follow_artist!(actor: user_two)
  ...
  describe "image_url" do
    test "uses the cover from the first album that has a cover" do
      artist = generate(artist())
      generate(album(artist_id: artist.id, year: 2021))

      generate(
        album(
          artist_id: artist.id,
          year: 2019,
          image_url: "/images/older.jpg"
        )
      )
      ...
      generate(
        album(
          artist_id: artist.id,
          year: 2020,
          image_url: "/images/the_real_cover.png"
        )
      )

      {:ok, artist} = Ash.load(artist, :image_url)
      assert artist.image_url == "/images/the_real_cover.png"
    end
  end
